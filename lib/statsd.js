...
Client.prototype.timing = function (stat, time, sample_rate, cb) {
    ...
    var stats = {};
    stats[stat] = time+"|ms";
    self.registerStat(stats, sample_rate, cb);
};
...
Client.prototype.update_stats = function (stats, delta, sampleRate, cb) {
    ...
    self.registerStat(data, sampleRate, cb);
}

Client.prototype.registerStat = function (stat, data, sample_rate, callback) {

    if (!this.commands[stat])
      this.commands[stat] = 0;

    if (this.commands[stat]++ == Math.round(1/sample_rate) - 1) {
      this.commands[stat] = 0;
      this.send(data, sample_rate, callback);
    }
}
